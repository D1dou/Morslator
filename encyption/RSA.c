...
void __RSA(char *message, int mode)
{
    ...
    #ifdef PRINT_KEYS
        FILE *out1 = fopen("public_key", "w");
        fwrite(pub_key, sizeof(*pub_key), pub_len, out1);
        fclose(out1);

        out1 = fopen("private_key", "w");
        fwrite(pri_key, sizeof(*pri_key), pri_len, out1);
        fclose(out1);
    #endif
                    ...
                    keypair, RSA_PKCS1_OAEP_PADDING)) == -1) 
    {
        ...
        printf("Reading the encrypted message and attempting decryption...\n");
        encrypt = malloc(RSA_size(keypair));
        out = fopen("out.txt", "r");
