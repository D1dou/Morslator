...
unsigned int default_sBox[4][256] =
{
    {
...
{
	unsigned int leftSide = *left, rightSide = *right;
	unsigned int temp;
	for (int i = 0; i < 16; ++i)
	{
		leftSide ^= ptr->p[i];
...
{
	unsigned int leftSide, rightSide, temp=0x00;
	for (int i = 0; i < 18; ++i)
	{
		ptr->p[i] = default_p[i];
	}
...
{
	unsigned int leftSide = *left, rightSide = *right, temp;
