...
{
	unsigned int bytes_for_secret_file_size = 4;
	if (encInfo->image_capacity >= (MAX_IMAGE_BUF_SIZE * (encInfo->passcode_size + MAX_FILE_SUFFIX + (unsigned int)sizeof(encInfo->extn_secret_file) + bytes_for_secret_file_size + encInfo->size_secret_file)))
	{
		return e_success;
	}
	return e_failure;
...
Status encode_magic_string(const char *magic_string, EncodeInfo *encInfo)
{
	...
	{
		for (unsigned int i = 0; i < encInfo->passcode_size; i++)
		{
			// copy * to secret_data
...
Status encode_secret_file_extn_size(EncodeInfo *encInfo)
{
	...
	unsigned int size_bits = MAX_IMAGE_BUF_SIZE * (unsigned int) sizeof(int);
	char data[size_bits];
	// Divide secrete_data to 8 bits i.e MSB at 0th index and LSB at 7th index
	for (unsigned i = 0; i < size_bits; i++)
	{
		if ((unsigned int) len & mask)
		{
			data[i] = 1;
		}
		else
...
{
	unsigned int mask = 0x80000000;
	unsigned int len = (unsigned int) encInfo->size_secret_file;
	unsigned int size_bits = MAX_IMAGE_BUF_SIZE * (unsigned int) sizeof(int);
	char data[size_bits];
...
Status encode_secret_file_data(EncodeInfo *encInfo)
{
	...
	fread(data, sizeof(char), (unsigned int) encInfo->size_secret_file, encInfo->fptr_secret);
...
Status encode_byte_to_lsb(char data, char *image_buffer)
{
	...
	for(unsigned int i = 0; i < MAX_IMAGE_BUF_SIZE; i++)
	{
		if ( data & mask)
		{
